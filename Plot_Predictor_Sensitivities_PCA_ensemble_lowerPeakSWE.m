...
best_mod_ids = readtable('/Volumes/Pruina_External_Elements/DroughtFireSnow/Data/AnlysisData/Best_Obs_Forecast_outputs/BAs_1984_2020_Obs_Forecast_SE_Zbins_SWEImod_PCA_bestmods_lowerPeakSWE.csv');
...
for i=1:nmods
    ...
    infilename=sprintf('/Volumes/Pruina_External_Elements/DroughtFireSnow/Data/AnlysisData/Best_Obs_Forecast_outputs/BAs_1984_2020_Obs_Forecast_SE_Zbins_SWEImod_PCA_drop_predictor_sensitivity_mod%d_lowerPeakSWE.csv',mod_id);
...
grid on
plot(linspace(0.5,length(idx_over10)+0.5,10),linspace(nanmedian(store_R2_ratios(:,2)),nanmedian(store_R2_ratios(:,2)),10),'--r')
plot(linspace(0.5,length(idx_over10)+0.5,10),linspace(prctile(store_R2_ratios(:,2),25),prctile(store_R2_ratios(:,2),25),10),'--b')
plot(linspace(0.5,length(idx_over10)+0.5,10),linspace(prctile(store_R2_ratios(:,2),75),prctile(store_R2_ratios(:,2),75),10),'--b')
...
xlim([0.5 length(idx_over10)+0.5])
grid on
plot(linspace(0.5,length(idx_over10)+0.5,10),linspace(nanmedian(store_rmse_ratios(:,2)),nanmedian(store_rmse_ratios(:,2)),10),'--r')
plot(linspace(0.5,length(idx_over10)+0.5,10),linspace(prctile(store_rmse_ratios(:,2),25),prctile(store_rmse_ratios(:,2),25),10),'--b')
plot(linspace(0.5,length(idx_over10)+0.5,10),linspace(prctile(store_rmse_ratios(:,2),75),prctile(store_rmse_ratios(:,2),75),10),'--b')

f.Position=[-1919        -144        1868         949];
saveas(f,'/Users/abolafia/Drought_Fire_Snow/Plots/BA_predict_1984_2020_Obs_SWEImod_PCA_predictor_sensitivity_ensemble_lowerPeakSWE.eps','epsc')
