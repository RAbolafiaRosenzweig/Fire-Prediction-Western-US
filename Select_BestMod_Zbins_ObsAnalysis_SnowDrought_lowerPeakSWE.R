...
#0-1100m:
Climate_Fire_Data = read.table("/Volumes/Pruina_External_Elements/DroughtFireSnow/Data/AnlysisData/Annual_Fire_Climate_Obs_forecast_Ziter1_lowerPeakSWE.csv",sep=",")
#define variables:
...
#1100-2200:
Climate_Fire_Data = read.table("/Volumes/Pruina_External_Elements/DroughtFireSnow/Data/AnlysisData/Annual_Fire_Climate_Obs_forecast_Ziter2_lowerPeakSWE.csv",sep=",")
#define variables:
#define variables:
WYs = Climate_Fire_Data[1:37,1]
BAs = Climate_Fire_Data[1:37,2]
...
#2200-3300:
Climate_Fire_Data = read.table("/Volumes/Pruina_External_Elements/DroughtFireSnow/Data/AnlysisData/Annual_Fire_Climate_Obs_forecast_Ziter3_lowerPeakSWE.csv",sep=",")
#define variables:
#define variables:
WYs = Climate_Fire_Data[1:37,1]
BAs = Climate_Fire_Data[1:37,2]
...
#   predictor10 <-Climate_Fire_DF[,col10]
#   predictor11 <-Climate_Fire_DF[,col11]
#   predictor12 <-Climate_Fire_DF[,col12]
#   
#   #PCA combo model:
#   Covariates_DF = data.frame(predictor1=predictor1,predictor2=predictor2,predictor3=predictor3,predictor4=predictor4,predictor5=predictor5,predictor6=predictor6,predictor7=predictor7,predictor8=predictor8,predictor9=predictor9,predictor10=predictor10,predictor11=predictor11,predictor12=predictor12)
#   df.pca <- prcomp(Covariates_DF, center = TRUE,scale. = TRUE)
#   pcs = df.pca$x
#   pc1 = pcs[,1]
#   pc2 = pcs[,2]
...
#   pc11 = pcs[,11]
#   pc12 = pcs[,12]
#   current_DF_pcs <- data.frame(Y=Y,predictor1=pc1,predictor2=pc2,predictor3=pc3,predictor4=pc4,predictor5=pc5,predictor6=pc6,predictor7=pc7,predictor8=pc8,predictor9=pc9,predictor10=pc10,predictor11=pc11,predictor12=pc12,predictor13=as.vector(Zs))
#   mod <- gam(data=current_DF_pcs,Y~s(predictor1)+s(predictor2)+s(predictor3)+s(predictor4)+s(predictor5)+s(predictor6)+s(predictor7)+s(predictor8)+s(predictor9)+s(predictor10)+s(predictor11)+s(predictor12)+predictor13,family="gaussian")
#   
#   X = concurvity(mod,full=FALSE)
#   X=X$estimate
#   idx <- which(X==1)
...
#     R = cor(predict(mod),Y)
#     
#     store_R_12vars = c(store_R_12vars,R)
#     store_AIC_12vars = c(store_AIC_12vars,AIC)
#     store_i_12vars = c(store_i_12vars,i)
#   }
# }
# 
# idx_bestmod_12vars <- which(store_AIC_12vars==min(store_AIC_12vars))
# store_R_12vars[idx_bestmod_12vars]
# idx_bestmod_12vars = store_i_12vars[idx_bestmod_12vars]
# store_best_mod_idx = cbind(idx_bestmod_2vars,idx_bestmod_3vars,idx_bestmod_4vars,idx_bestmod_5vars,idx_bestmod_6vars,idx_bestmod_7vars,idx_bestmod_8vars)
# 
# idx_bestmod_2vars <- which(store_AIC_2vars==min(store_AIC_2vars))
# idx_bestmod_3vars <- which(store_AIC_3vars==min(store_AIC_3vars))
# idx_bestmod_4vars <- which(store_AIC_4vars==min(store_AIC_4vars))
...
# idx_bestmod_7vars <- which(store_AIC_7vars==min(store_AIC_7vars))
# idx_bestmod_8vars <- which(store_AIC_8vars==min(store_AIC_8vars))
# store_best_R = cbind(store_R_2vars[idx_bestmod_2vars],store_R_3vars[idx_bestmod_3vars],store_R_4vars[idx_bestmod_4vars],store_R_5vars[idx_bestmod_4vars],store_R_6vars[idx_bestmod_6vars],store_R_7vars[idx_bestmod_7vars],store_R_8vars[idx_bestmod_8vars])
# store_best_AIC = cbind(store_AIC_2vars[idx_bestmod_2vars],store_AIC_3vars[idx_bestmod_3vars],store_AIC_4vars[idx_bestmod_4vars],store_AIC_5vars[idx_bestmod_5vars],store_AIC_6vars[idx_bestmod_6vars],store_AIC_7vars[idx_bestmod_7vars],store_AIC_8vars[idx_bestmod_8vars])
# 
# 
# idx_bestmod_2vars = store_i_2vars[idx_bestmod_2vars]
# idx_bestmod_3vars = store_i_3vars[idx_bestmod_3vars]
# idx_bestmod_4vars = store_i_4vars[idx_bestmod_4vars]
# idx_bestmod_5vars = store_i_5vars[idx_bestmod_5vars]
# idx_bestmod_6vars = store_i_6vars[idx_bestmod_6vars]
# idx_bestmod_7vars = store_i_7vars[idx_bestmod_7vars]
# idx_bestmod_8vars = store_i_8vars[idx_bestmod_8vars]
...
best_is=store_i
outfilename = sprintf("/Volumes/Pruina_External_Elements/DroughtFireSnow/Data/AnlysisData/Best_Obs_Forecast_outputs/BAs_1984_2020_Obs_Forecast_SE_Zbins_SWEImod_PCA_bestmods_lowerPeakSWE.csv")
